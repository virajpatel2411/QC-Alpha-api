...
class Dashboard1 extends Component {
  state = {
    ...
    flag: true,
    responseData: "",
    hideColumnList: [false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false],
    totHiddenColumns: 0,
  };
  ...
  saveAccessToken = () => {
    ...
    if (reactLocalStorage.get("accessToken") && reactLocalStorage.get("API_Key")) {
      axios
        .get(
          `https://qcalpha-dashboard.herokuapp.com/instruments?accessToken=${reactLocalStorage.get("accessToken")}&API_Key=${reactLocalStorage.get("API_Key")}`
        )
        .then((res) => {
          console.log(res);
     ...
     <div>
     {
        this.colNames.map((val,ind) => (
          <h5 key={ind}
              ...
              onClick={(event) => {this.updateHideColumnList(event,ind,false)}}
          >
  ...
  getTableStructure = () => {
    ...
    return (
      <div>
        <Table striped bordered size="sm"/*className="table table-bordered table-striped"*/>
          <thead>
            <tr>
  ...
  componentDidMount = () => {
    ...
    if (reactLocalStorage.get("accessToken") && reactLocalStorage.get("API_Key")) {
      axios
        .get(
          `https://qcalpha-dashboard.herokuapp.com/expiry?accessToken=${reactLocalStorage.get("accessToken")}&API_Key=${reactLocalStorage.get("API_Key")}`
        )
        .then((res) => {
          //console.log(res);
      ...
      <div>
        ...
        >
          <h5>
            ...
            >
              <Badge variant="info" style={{display: 'inline'} }>𝒊</Badge>
            </OverlayTrigger>
